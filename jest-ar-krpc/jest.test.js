...
describe ("moneyMoves", () => {
  it("returns in budget if price is < 300", () => {
    expect(moneyMoves).toEqual("in budget")
    expect(moneyMoves).toEqual("over budget")
  })
})

const moneyMoves = (number) => {
  if (number < 300){
    return "in budget"
  } else {
    return "over budget"
...
describe ("numberMachine", () => {
  it("takes two numbers and returns the smaller", () => {
    if (num1 > num2){
      return num2
    } else if (num2 > num1){
      return num1
    ...
    expect(numberMachine()).toBeLessThan()
  })
})
